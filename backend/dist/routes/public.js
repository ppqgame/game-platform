...
/** 前台只展示已发布且未软删的游戏 */
const liveGame = { status: "PUBLISHED", deletedAt: null };
const paginationSchema = z.object({
    page: z.coerce.number().int().positive().default(1),
    pageSize: z.coerce.number().int().positive().max(50).default(20),
});
function normalizePlayableUrlForResponse(v) {
    if (!v)
        return v;
    if (v.startsWith("/games/"))
        return v.replace(/^\/games\//, "/play/");
    if (/^https?:\/\//i.test(v)) {
        try {
            const u = new URL(v);
...
publicRouter.get("/discover", async (req, res) => {
    const query = z
        ...
        .parse(req.query);
    const where = { ...liveGame };
    if (query.category)
        where.category = { slug: query.category };
    if (query.tags.length)
        where.tags = { some: { tag: { slug: { in: query.tags } } } };
    if (query.age && query.age !== "all") {
        const map = { "12+": "P12", "16+": "P16", "18+": "P18" };
        const ar = map[query.age];
        if (ar)
            where.ageRating = ar;
    }
    const orderBy = query.sort === "new" ? { publishedAt: "desc" } : query.sort === "plays" ? { launchCount: "desc" } : { updatedAt: "desc" };
    const [total, games] = await prisma.$transaction([
        prisma.game.count({ where }),
        prisma.game.findMany({
            where,
...
function mapPublicGameDetail(game) {
    ...
    const iconAssets = game.assets
        ...
        .map((a) => ({ kind: "ICON", url: a.url, alt: a.alt }));
    const bannerAssets = game.assets
        .filter((a) => a.type === "BANNER")
        .map((a) => ({ kind: "BANNER", url: a.url, alt: a.alt }));
    const screenshotAssets = game.assets
        .filter((a) => a.type === "SCREENSHOT")
        .sort((a, b) => a.sortOrder - b.sortOrder)
        .map((a) => ({ kind: "SCREENSHOT", url: a.url, alt: a.alt }));
...
function mapPublicGameCard(game) {
