...
function isSafeZipEntry(name: string) {
...
function detectPlayableEntry(files: string[]) {
...
/** 生成可用于目录名的字符串，保留大小写。 */
function toCasePreservingFolderName(input: string) {
...
function isHttpOrUploadPath(s: string) {
...
function isAllowedPlayUrl(s: string): boolean {
  const t = s.trim();
  if (!t) return false;
...
type PendingReviewStatus = "REVIEW" | "TESTING";
type GameWritePayload = {
  ...
  ageRating?: "ALL" | "P12" | "P16" | "P18" | null;
  screenOrientation?: "LANDSCAPE" | "PORTRAIT";
  isHot?: boolean;
  isHomeRecommended?: boolean;
  titleI18nJson?: string | null;
  extConfigJson?: string | null;
};
...
function pendingUpdateFile(gameId: string) {
  return path.join(pendingUpdatesDir, `${gameId}.json`);
}

function readPendingUpdate(gameId: string): PendingGameUpdate | null {
  const file = pendingUpdateFile(gameId);
  if (!fs.existsSync(file)) return null;
  try {
    const raw = JSON.parse(fs.readFileSync(file, "utf8")) as PendingGameUpdate;
    if (!raw || (raw.reviewStatus !== "REVIEW" && raw.reviewStatus !== "TESTING") || !raw.payload) return null;
    return raw;
  } catch {
    return null;
  }
...
function writePendingUpdate(gameId: string, value: PendingGameUpdate) {
  fs.writeFileSync(pendingUpdateFile(gameId), JSON.stringify(value, null, 2), "utf8");
}

function clearPendingUpdate(gameId: string) {
  const file = pendingUpdateFile(gameId);
  if (fs.existsSync(file)) fs.unlinkSync(file);
}

const playUrlField = z.preprocess(
  (v) => {
  ...
  z
    ...
    .refine(
      (v) => v === undefined || v === null || (typeof v === "string" && isAllowedPlayUrl(v)),
      { message: "须为 http(s)、以 / 开头的路径或相对路径" }
    )
);
...
const mediaUrl = z.preprocess(
  ...
  z
    .union([z.string(), z.null(), z.undefined()])
    .refine(
      (v) => v === undefined || v === null || isHttpOrUploadPath(v),
      { message: "须为 http(s) 或 /uploads/ 下的地址" }
    )
);
...
adminRouter.post(
  ...
  requireAdmin,
  (req, res, next) => {
    gameZipUpload.single("file")(req, res, (err: unknown) => {
      if (err) {
        if (err instanceof MulterError && err.code === "LIMIT_FILE_SIZE") {
  ...
  async (req, res) => {
    if (!req.file) throw new HttpError(400, "请选择要上传的 ZIP 包");
    ...
    if (!gameFolder) throw new HttpError(400, "无法生成游戏目录名");
    ...
    try {
      ...
      if (!entries.length) throw new HttpError(400, "ZIP 包为空");
...
adminRouter.post("/auth/refresh", async (req, res) => {
  ...
  if (!session || session.expiresAt.getTime() < Date.now()) throw new HttpError(401, "Refresh token expired");
...
adminRouter.patch("/games/:id", requireAdmin, async (req, res) => {
  ...
  if (!existingGame) throw new HttpError(404, "Game not found");
  if (existingGame.status === "PUBLISHED") {
    const current = readPendingUpdate(id);
    const currentPayload: Partial<GameWritePayload> = current?.payload ?? {};
    const mergedPayload: GameWritePayload = {
      title: (body.title ?? currentPayload.title ?? existingGame.title) as string,
      shortPitch: (body.shortPitch ?? currentPayload.shortPitch ?? existingGame.shortPitch) as string,
      description: (body.description ?? currentPayload.description ?? existingGame.description) as string,
      categorySlug: (body.categorySlug ?? currentPayload.categorySlug) as string | undefined,
      tagSlugs: (body.tagSlugs ?? currentPayload.tagSlugs ?? []) as string[],
      playUrl:
        body.playUrl === undefined
          ? (currentPayload.playUrl ?? existingGame.playUrl ?? null)
          : (normalizePlayUrlForStorage(body.playUrl) ?? null),
      embedUrl: (body.embedUrl === undefined ? currentPayload.embedUrl ?? existingGame.embedUrl ?? null : body.embedUrl ?? null) as
        | string
        | null,
      coverUrl: (body.coverUrl === undefined ? (currentPayload.coverUrl ?? null) : body.coverUrl ?? null) as string | null,
      bannerUrl: (body.bannerUrl === undefined ? (currentPayload.bannerUrl ?? null) : body.bannerUrl ?? null) as string | null,
      screenshotUrls: (body.screenshotUrls ?? currentPayload.screenshotUrls ?? []) as string[],
      developer:
        (body.developer === undefined ? currentPayload.developer ?? existingGame.developer ?? null : body.developer ?? null) as
          | string
          | null,
      ageRating: (body.ageRating === undefined ? currentPayload.ageRating ?? existingGame.ageRating ?? "ALL" : body.ageRating ?? null) as
        | "ALL"
        | "P12"
        | "P16"
        | "P18"
        | null,
      screenOrientation:
        (body.screenOrientation === undefined
      ...
      isHomeRecommended:
        body.isHomeRecommended === undefined
          ? currentPayload.isHomeRecommended ?? existingGame.isHomeRecommended
          : body.isHomeRecommended,
      titleI18nJson:
        (body.titleI18nJson === undefined ? currentPayload.titleI18nJson ?? existingGame.titleI18nJson : body.titleI18nJson ?? "{}") ??
        "{}",
      extConfigJson:
        (body.extConfigJson === undefined ? currentPayload.extConfigJson ?? existingGame.extConfigJson : body.extConfigJson ?? "{}") ??
        "{}",
    };
  ...
  const updated = await prisma.$transaction(async (tx) => {
    ...
    if (body.categorySlug !== undefined) {
      if (!body.categorySlug) categoryId = null;
      else {
    ...
    if (body.tagSlugs) await syncTags(tx, id, body.tagSlugs);
    ...
    if (body.coverUrl !== undefined || body.bannerUrl !== undefined || body.screenshotUrls !== undefined) {
      ...
      if (body.screenshotUrls !== undefined) patch.screenshotUrls = body.screenshotUrls;
...
adminRouter.post("/games/:id/publish", requireAdmin, async (req, res) => {
  ...
  const game = await prisma.$transaction(async (tx) => {
    ...
    if (!existing) throw new HttpError(404, "Game not found");
    if (existing.deletedAt) throw new HttpError(400, "已软删的游戏需先恢复后再发布");

    if (existing.status === "PUBLISHED") {
      const pending = readPendingUpdate(id);
      if (!pending) throw new HttpError(400, "该游戏没有待发布修改");
      if (pending.reviewStatus !== "TESTING") throw new HttpError(400, "待修改版本需先进入测试状态");
    ...
    if (existing.status !== "TESTING") throw new HttpError(400, "仅测试状态可发布，请先通过测试流程");

    if (!existing.playUrl) throw new HttpError(400, "playUrl is required to publish");

    const hasCover = existing.assets.some((a) => a.type === "COVER");
    if (!hasCover) throw new HttpError(400, "coverUrl is required to publish");
...
adminRouter.post("/games/:id/submit-review", requireAdmin, async (req, res) => {
  ...
  if (g.deletedAt) throw new HttpError(400, "已删除");
  if (g.status === "PUBLISHED") {
    const pending = readPendingUpdate(id);
    if (!pending) throw new HttpError(400, "该已发布游戏没有待审核修改");
  ...
  if (g.status !== "DRAFT") throw new HttpError(400, "仅草稿可提交审核");
...
adminRouter.post("/games/:id/start-testing", requireAdmin, async (req, res) => {
  ...
  if (g.status === "PUBLISHED") {
    const pending = readPendingUpdate(id);
    if (!pending) throw new HttpError(400, "该已发布游戏没有待测试修改");
  ...
  if (g.status !== "REVIEW" && g.status !== "DRAFT") throw new HttpError(400, "需从草稿或待审核进入测试");
...
adminRouter.post("/games/:id/restore", requireAdmin, async (req, res) => {
  ...
  const g = await prisma.game.findUnique({ where: { id } });
  if (!g) throw new HttpError(404, "Game not found");
  if (!g.deletedAt) throw new HttpError(400, "未在回收站中");
...
async function syncTags(tx: Prisma.TransactionClient, gameId: string, tagSlugs: string[]) {
...
async function syncAssets(tx: Prisma.TransactionClient, gameId: string, body: AssetPatch) {
...
type AdminGamePayload = Prisma.GameGetPayload<{
  include: {
    category: true;
    tags: { include: { tag: true } };
    assets: true;
...
function mapAdminGame(game: AdminGamePayload) {
